...
class SnipOverlay(tk.Toplevel):
    ...
    def on_release(self, event):
        ...
        # DPI-safe conversion:
        # overlay coordinates are logical/display coords
        # screenshot size is real pixel size
        ...
        cropped = self.screenshot.crop((crop_left, crop_top, crop_right, crop_bottom))
        self.destroy()
...
class Editor(tk.Frame):
    def __init__(self, master):
        ...
        self.master.after(250, self.start_snip)
    ...
    def build_ui(self):
        ...
        image_frame = tk.Frame(body, bg=PANEL_DARK, bd=0, highlightthickness=1, highlightbackground='#444444')
        ...
        toolbar = tk.Frame(bottom, bg=PANEL_DARK, bd=0, highlightthickness=1, highlightbackground='#444444')
        ...
        self.make_tool_button(left, '✂  New Snip', self.start_snip, ACCENT_GREY).pack(side='left', padx=4)
        self.make_tool_button(left, '🖼  Open Image', self.load_image, ACCENT_GREY).pack(side='left', padx=4)
        self.make_tool_button(left, '✏  Add Text', lambda: self.set_mode('text'), ACCENT_BLUE).pack(side='left', padx=4)
        self.make_tool_button(left, '⬜  Add Box', lambda: self.set_mode('rect'), ACCENT_BROWN).pack(side='left', padx=4)
        self.make_tool_button(left, '↩  Undo', self.undo_last, ACCENT_GREY).pack(side='left', padx=4)
        self.make_tool_button(left, '🗑  Clear All', self.clear_all, ACCENT_GREY).pack(side='left', padx=4)

        self.make_tool_button(right, '📋  Copy to Clipboard & Exit', self.copy_and_exit, ACCENT_GREEN).pack(side='left', padx=4)
        self.make_tool_button(right, '💾  Save PNG', self.save_png, ACCENT_GREY).pack(side='left', padx=4)

        options = tk.Frame(bottom, bg=PANEL_DARK, bd=0, highlightthickness=1, highlightbackground='#444444')
        options.pack(fill='x')

        self.mode_label = tk.Label(options, text='Current Mode: Box', bg=PANEL_DARK, fg=TEXT_LIGHT, font=('Segoe UI', 10))
        self.mode_label.pack(side='left', padx=(12, 14), pady=10)

        tk.Label(options, text='Box Width', bg=PANEL_DARK, fg=TEXT_LIGHT, font=('Segoe UI', 10)).pack(side='left', padx=(0, 6))
        rect_spin = tk.Spinbox(options, from_=1, to=12, width=4, command=lambda: self.update_rect_width(rect_spin.get()))
        ...
        tk.Label(options, text='Text Size', bg=PANEL_DARK, fg=TEXT_LIGHT, font=('Segoe UI', 10)).pack(side='left', padx=(0, 6))
        text_spin = tk.Spinbox(options, from_=10, to=96, textvariable=self.text_size, width=4, command=self.on_text_size_change)
        ...
        tk.Label(options, text='Scale %', bg=PANEL_DARK, fg=TEXT_LIGHT, font=('Segoe UI', 10)).pack(side='left', padx=(0, 6))
        dpi_spin = tk.Spinbox(options, from_=50, to=300, textvariable=self.display_scale_percent, width=4, command=self.apply_display_scale)
        dpi_spin.pack(side='left', padx=(0, 10), pady=8)
        dpi_spin.bind('<KeyRelease>', lambda e: self.apply_display_scale())

        self.status_label = tk.Label(bottom, textvariable=self.status_text, anchor='w', bg=BG_DARK, fg='#d8d8d8', font=('Segoe UI', 10))
    ...
    def apply_display_scale(self):
        try:
            ...
            value = detected if self.auto_display_scale.get() else max(50, min(300, int(self.display_scale_percent.get())))
            self.display_scale_percent.set(value)
            if sys.platform.startswith('win'):
                try:
                    self.master.tk.call('tk', 'scaling', value / 75.0)
                except Exception:
                    pass
            self.refresh_canvas()
    ...
    def set_mode(self, mode):
        ...
        if mode == 'rect':
            ...
            self.status_text.set('Box mode: Drag on image to add red box.')
        else:
    ...
    def on_text_size_change(self):
        ...
        self.status_text.set(f'Text size set to {max(10, int(self.text_size.get()))}.')
    ...
    def refresh_canvas(self):
        ...
        self.scale = scale
        self.preview_image = rendered.resize((pw, ph), Image.LANCZOS) if scale != 1 else rendered
    ...
    def render_result_image(self, include_edit_preview=False):
        ...
        for action in self.actions:
            if action['type'] == 'rect':
                draw.rectangle(action['bbox'], outline=action['color'], width=action['width'])
            elif action['type'] == 'text':
                font = get_font(size=action['size'])
                draw.multiline_text(action['position'], action['text'], fill=action['color'], font=font, spacing=6)
        if include_edit_preview and self.active_text_position and self.active_text_var is not None:
            ...
            if preview:
                ...
                draw.multiline_text(self.active_text_position, preview, fill=self.text_color, font=font, spacing=6)
        return img
    ...
    def canvas_press(self, event):
        ...
        self.current_rect_id = self.canvas.create_rectangle(event.x, event.y, event.x, event.y, outline=self.rect_color, width=self.rect_width)
    ...
    def start_inline_text(self, event):
        ...
        self.active_text_window_id = self.canvas.create_window(preview_x, preview_y, anchor='nw', window=self.active_text_entry, width=260)
    ...
    def update_active_text_entry_style(self):
        if self.active_text_entry is not None:
            ...
            self.active_text_entry.configure(font=('Arial', font_size), fg=self.text_color, insertbackground=self.text_color)
    ...
    def pick_rect_color(self):
        color = colorchooser.askcolor(color=self.rect_color, title='Pick rectangle color')[1]
        if color:
            self.rect_color = color
            self.status_text.set('Box color updated.')

    def pick_text_color(self):
        color = colorchooser.askcolor(color=self.text_color, title='Pick text color')[1]
        if color:
